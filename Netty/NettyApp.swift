...
//            await randomAccounts()
    ...
    private func randomAccounts() async {
        for _ in 1...50 {
            ...
            var nickname = "\(firstName)_\(lastName)".lowercased()
            ...
            case .success(let record):
                print("Success \(record.recordID)")
                switch await PostsService.instance.addPostForUserWith(record.recordID, image: UIImage(named: "testImage")!) {
                case .success(let post):
                    print("Success post \(post.id)")
